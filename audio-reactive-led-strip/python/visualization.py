...
def visualize_spectrum(y):
    ...
    r = np.array([0] * len(diff)) # r_filt.update(np.copy(y))
    g = g_filt.update(np.copy(y)).clip(0.2, None) #np.array([255] * len(diff)) #np.abs(diff)

    # g = g
    # g = np.zeros(len(diff))
    b = b_filt.update(y - common_mode.value)
    g[b > 0.5] *= 0.5
